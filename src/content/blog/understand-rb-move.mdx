...
import LineGraph from "../../components/LineChart.tsx";
...
<LineGraph client:load traces={[tsu_trace, ver_trace]} />
<LineGraph client:load traces={[law_trace, ver_trace]} />
...
<LineGraph client:load traces={[tsu_brake_trace, ver_brake_trace]} />
<LineGraph client:load traces={[law_brake_trace, ver_brake_trace]} />

With the braking, Lawson is clearly shown as a late breaker, with a significant
emphasis on shorter, later brake windows. However, this further proves the point
that the style Lawson brings is just not suited towards the car. Verstappen
prefers earlier braking, with more focus on using the brakes sporadically rather
than as a continuous force. This identically matches Tsunoda’s driving style as
shown on the right.
